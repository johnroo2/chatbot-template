...
export default function Signup({ setUser }: BaseProps) {
	...
	const onSubmit = async () => {
		...
		const res = await userService.signup(username, password.trim(), false);

		if (res instanceof AxiosError) {
			toast.error('Signup failed', {
				description: res?.response?.data?.message || 'An unexpected error occured while attempting signup',
			});
		} else {
...
Signup.breadcrumb = JSON.stringify([{ name: 'Signup', isLink: true, link: '/signup' }] as BreadcrumbType[]);